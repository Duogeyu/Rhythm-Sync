...
try {
    ...
    for (const userSong of userSongs) {
        ...
        if (fuzzyResults.length > 0) {
            ...
            // Normalize score from fuzzysort (-1000 to 0) to (0 to 1)
            const normalizedScore = Math.max(0, (result.score + 1000) / 1000);

            // Map the old fuse threshold logic
            if (normalizedScore > 0.7) { // fuse score < 0.3
                matches.push({
                    userSong,
                    arcadeSong: result.obj.original,
                    score: normalizedScore,
                    matchType: normalizedScore > 0.9 ? 'exact' : 'fuzzy' // fuse score < 0.1
                });
            }
        }
    }
