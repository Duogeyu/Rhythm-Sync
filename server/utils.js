// 预编译正则表达式以优化性能
const REGEX_SPACES = /\s+/g;
const REGEX_EXCLAMATION_FULL = /[！!]/g;
const REGEX_QUESTION_FULL = /[？?]/g;
const REGEX_PARENTHESIS_LEFT = /[（(]/g;
const REGEX_PARENTHESIS_RIGHT = /[）)]/g;
const REGEX_DASH = /[－-]/g;

// 标准化标题用于精确匹配
const normalizeTitle = (str) => {
    if (!str) return '';
    return str.toLowerCase()
        .replace(REGEX_SPACES, '')
        .replace(REGEX_EXCLAMATION_FULL, '!')
        .replace(REGEX_QUESTION_FULL, '?')
        .replace(REGEX_PARENTHESIS_LEFT, '(')
        .replace(REGEX_PARENTHESIS_RIGHT, ')')
        .replace(REGEX_DASH, '-');
};

const REGEX_ARTIST_SEPARATORS = /[,，、&＆×x]/g;
const REGEX_FEAT = /feat\.?/gi;
const REGEX_CV = /cv[.:]?/gi;
const REGEX_PARENTHESIS_CONTENT = /[(（][^)）]*[)）]/g;

// 辅助函数：计算艺术家相似度
const normalizeArtist = (str) => {
    if (!str) return '';
    return str.toLowerCase()
        .replace(REGEX_SPACES, '')
        .replace(REGEX_ARTIST_SEPARATORS, '')
        .replace(REGEX_FEAT, '')
        .replace(REGEX_CV, '')
        .replace(REGEX_PARENTHESIS_CONTENT, '');
};

module.exports = {
    normalizeTitle,
    normalizeArtist
};
