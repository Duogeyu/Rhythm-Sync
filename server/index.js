...
function isSafeFilename(filename) {
    ...
    // Also validate it's purely alphanumeric or has safe characters for ids/filenames.
    // E.g. allow alphanumeric, hyphens, underscores, dots.
...
app.get('/api/covers/:gameId/:fileName', async (req, res) => {
    ...
    if (!isSafeFilename(gameId) || !isSafeFilename(fileName)) {
        return res.status(400).json({ error: '无效的文件名或游戏 ID' });
    }
...
app.get('/api/share/:shareId', (req, res) => {
    try {
        ...
        if (!isSafeFilename(shareId)) {
            return res.status(400).json({ success: false, error: 'invalid', message: '无效的分享 ID' });
        }
