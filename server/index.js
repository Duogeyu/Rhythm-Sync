...
app.get('/api/covers/:gameId/:fileName', async (req, res) => {
    ...
    if (!isSafeFilename(gameId) || !isSafeFilename(fileName)) return res.status(400).json({ error: 'Invalid filename' });
...
app.get('/api/random/image/:id', (req, res) => {
    const { id } = req.params;
    if (!isSafeFilename(id)) return res.status(400).json({ error: 'Invalid id' });
...
app.get('/api/share/:shareId', (req, res) => {
    try {
        ...
        if (!isSafeFilename(shareId)) return res.status(400).json({ error: 'Invalid shareId' });
        const filePath = path.join(SHARE_DATA_DIR, `${shareId}.json`);
