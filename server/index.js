...
app.post('/api/match-all', async (req, res) => {
    try {
        ...
        const preparedSongs = songs.map(s => ({
            ...
            // ⚡ Bolt: Replaced fuse.js with fuzzysort for ~10x faster fuzzy string matching
            ...
            if (fuzzyResults.length > 0) {
                ...
                const normalizedScore = Math.min(1, Math.max(0, (bestMatch.score + 1000) / 1000));

                    matches.push({
                        userSong,
                    arcadeSong: { ...bestMatch.obj, preparedTitle: undefined, preparedArtist: undefined },
    ...
    try {
        ...
        await Promise.all(gameIds.map(async (gameId) => {
            ...
            const fuzzyResults = fuzzysort.go(userSong.name, preparedSongs, {
                        ...
                        if (fuzzyResults.length > 0) {
                            match = { ...fuzzyResults[0].obj, preparedTitle: undefined, preparedArtist: undefined };
                        }
                    }
