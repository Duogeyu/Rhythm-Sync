        ...
        const matchPromises = gameDataResults.map(async ({ gameId, songs, error }) => {
            ...
            for (const userSong of userSongs) {
                ...
                if (fuzzyResults.length > 0) {
                    ...
                    const mappedScore = Math.max(0, (topMatch.score + 1000) / 1000);
                    matches.push({
                        userSong,
                        arcadeSong: topMatch.obj.original,
                        score: mappedScore,
                        matchType: mappedScore > 0.9 ? 'exact' : 'fuzzy'
                    });
                    matchedUserSongIds.add(userSong.id);
                }
            }
    ...
    try {
        ...
        await Promise.all(gameIds.map(async (gameId) => {
            try {
                ...
                const preparedGameSongs = gameSongs.map(s => ({
                    original: s,
                    preparedTitle: fuzzysort.prepare(s.title || ''),
                    preparedArtist: fuzzysort.prepare(s.artist || '')
                }));
                ...
                for (const userSong of userSongs) {
                    ...
                    // 模糊匹配
                    if (!match) {
                        const fuzzyResults = fuzzysort.go(userSong.name, preparedGameSongs, {
                            keys: ['preparedTitle', 'preparedArtist'],
                            limit: 1,
                            threshold: -300
                        });
