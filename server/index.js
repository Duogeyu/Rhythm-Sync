...
const levenshteinDistance = (s1, s2) => {
    ...
    for (let i = 1; i <= len2; i++) {
        ...
        for (let j = 1; j <= len1; j++) {
            const currentDiagonal = LEV_BUFFER[j];

            if (char2 === str1.charCodeAt(j - 1)) {
            ...
            LEV_BUFFER[j] = current;
        }
    }

    return LEV_BUFFER[len1];
};
