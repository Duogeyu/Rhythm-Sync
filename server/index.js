...
app.post('/api/match-all', async (req, res) => {
    try {
        ...
        const matchPromises = gameDataResults.map(async ({ gameId, songs, error }) => {
            ...
            // 2. 建立 fuzzysort 模糊匹配索引 (替代性能较差的 Fuse.js)
            const preparedSongs = songs.map(s => ({
                original: s,
                preparedTitle: fuzzysort.prepare(s.title || ''),
            ...
            for (const userSong of userSongs) {
                ...
                const fuzzyResults = fuzzysort.go(userSong.name, preparedSongs, {
                    ...
                    limit: 1,
                    threshold: -300 // 对应 fuse 的 0.3
                });

                if (fuzzyResults.length > 0) {
                    const bestMatch = fuzzyResults[0];
                    // 将 fuzzysort 分数 (-300~0) 归一化为 0~1 的相似度分数
                    const matchScore = Math.max(0, (bestMatch.score + 1000) / 1000);
                ...
                for (const userSong of userSongs) {
                    ...
                    if (!match) {
                        const fuzzyResults = fuzzysort.go(userSong.name, preparedSongs, {
                            keys: ['preparedTitle', 'preparedArtist'],
                            limit: 1,
                            threshold: -300 // 对应 fuse 的 0.3 阈值
                        });
