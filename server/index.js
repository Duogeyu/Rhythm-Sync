        ...
        const matchPromises = gameDataResults.map(async ({ gameId, songs, error }) => {
            ...
            for (const userSong of userSongs) {
                ...
                const results = fuzzysort.go(userSong.name, preparedSongs, {
                    ...
                    threshold: -300 // -300 corresponds to legacy fuse.js threshold of 0.3
                });

                if (results.length > 0) {
                    const bestMatch = results[0];
                    // Normalize native negative scoring to a 0-1 scale
                    const score = Math.max(0, (bestMatch.score + 1000) / 1000);

                    matches.push({
                        userSong,
                        arcadeSong: bestMatch.obj.original,
                        score: score,
                        matchType: bestMatch.score > -100 ? 'exact' : 'fuzzy' // -100 corresponds to legacy 0.1
                    });
                    matchedUserSongIds.add(userSong.id);
                }
            }
            ...
            try {
                ...
                for (const userSong of userSongs) {
                    ...
                    if (!match) {
                        const results = fuzzysort.go(userSong.name, preparedSongs, {
                            keys: ['preparedTitle', 'preparedArtist'],
                            threshold: -300, // corresponds to 0.3
                            limit: 1
                        });

                        if (results.length > 0) {
                            match = results[0].obj.original;
                        }
                    }
