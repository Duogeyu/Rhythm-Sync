...
const artistMatch = (userArtist, gameArtist) => {
    ...
    const uaParts = ua.split(REGEX_ARTIST_NON_ALPHANUM).filter(p => p.length >= 3);
    const gaParts = ga.split(REGEX_ARTIST_NON_ALPHANUM).filter(p => p.length >= 3);
...
function stringSimilarity(a, b) {
    ...
    const bigrams = (s) => {
        ...
        for (let i = 0; i < s.length - 1; i++) {
            // Pack two 16-bit char codes into one 32-bit integer to avoid string allocation
