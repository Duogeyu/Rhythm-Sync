...
## 2024-05-12 - [Optimize Bigram Generation in Jaccard Similarity]
**Learning:** Generating string bigrams using `.substring()` within hot loops (like in Jaccard similarity functions) creates many temporary string objects, significantly increasing Garbage Collection (GC) overhead in Node.js.
**Action:** Use bitwise packing of character codes `((s.charCodeAt(i) << 16) | s.charCodeAt(i + 1))` instead of `s.substring(i, i + 2)` to represent bigrams as integers, avoiding string allocations entirely.
